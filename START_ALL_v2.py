...
for files_dict in all_inputs:
    ...
    if grid_overlay:
        raw_times, raw_data, locations, header = cr.import_data_grid(raw_signal, raw_xy)
    ...
    if sine_fitting == False:
        sine_times = final_times
        sine_data = final_data
        phase_data = rhythmic_or_not
        refphases = rhythmic_or_not
        periods = rhythmic_or_not
        amplitudes = rhythmic_or_not
        decays = rhythmic_or_not
        r2s = rhythmic_or_not
        meaningful_phases = rhythmic_or_not
...
if sine_fitting == False:
    ...
    modified_df.to_csv(f'{mydir}{INPUT_FILES[0]}_signal_modified.csv', index=False)
    ...
    data.drop(columns=['Rhythmic', 'Phase',	'Period', 'Amplitude', 'Decay', 'Rsq']).to_csv(f'{mydir}{INPUT_FILES[0]}_decay_params.csv', index=False)
