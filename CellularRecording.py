...
def import_data_grid(raw_signal, raw_xy):
    """
    Import signal and XY data generated by Fiji GridOverlay macros
    """
    timer = plo.laptimer()

    print("Importing GridOverlay data... time: ",) 
    header_str =  np.genfromtxt(raw_signal, dtype='unicode', delimiter=',')[0, :]
    header = np.arange(1, len(header_str[1:])+1, 1)        #create sequential int names
    raw_times = np.genfromtxt(raw_signal, delimiter=',')[1:, 0]
    raw_data = np.genfromtxt(raw_signal, delimiter=',')[1:, 1:]
    locations = np.genfromtxt(raw_xy, delimiter=',')[1:, 1:]
...
def eigensmooth(times, detrended_data, ev_threshold=0.05, dim=40, min_ev=2):
    ...
    for data_idx, d0 in enumerate(detrended_data.T):
        ...
        # this catches it
        # truncate the first 24h during alignment  # Why? This was causing problems when fittng short traces.
        align, atype = alignment(d1, denoised, d=dim, dstart=0)  # EDIT HERE, originally dstart=96. Maybe bypass this as with savgol?
